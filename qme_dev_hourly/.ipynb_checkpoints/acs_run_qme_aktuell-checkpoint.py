...
outdir = "/g/data/xv83/users/at2708/bias_adjustment_acs_qme/qme_dev_copy_hourly/outputs_netcdf20_normalsr/"
# outdir = "/g/data/ia39/australian-climate-service/release/CORDEX/output-Adjust/CMIP6/bias-adjusted-output/AUST-20i/"
...
if __name__ == "__main__":
    ...
    if var in ("tasmax", "tasmin", "tas"):
        ...
        # mdl_raw_means = xr.open_mfdataset(fut_file_list, engine='h5netcdf',chunks={"time": chunk_time, "lat": chunk_lat, "lon": chunk_lon}, preprocess=preproc_find_mean, parallel = True).persist() #Alicia , engine='h5netcdf',chunks={"time": chunk_time, "lat": chunk_lat, "lon": chunk_lon}

######## Alicia
        # def preproc_clean(ds):
        #     return ds.drop_vars([item for item in ('height', 'lat_bnds', 'lon_bnds', 'time_bnds') if item in ds.variables or item in ds.dims])
        
        # mdl_raw_means = xr.open_mfdataset(
        #     fut_file_list,
        ...
        # Load all files without aggregating
        mdl_raw_means = xr.open_mfdataset(
            fut_file_list,
            parallel=True,
            engine='h5netcdf',
        ...
        mdl_raw_means = mdl_raw_means.drop_vars(bnds_list).groupby("time.year").mean("time")
...
####### end Alicia
    ...
    ref_data_full = xr.open_mfdataset(ref_file_list, engine='h5netcdf',chunks={"time": chunk_time, "lat": chunk_lat, "lon": chunk_lon}, preprocess = preprocess_ds, parallel = True) #Alicia
    hist_data_full = xr.open_mfdataset(hist_file_list, engine='h5netcdf',chunks={"time": chunk_time, "lat": chunk_lat, "lon": chunk_lon}, parallel = True) #Alicia
    ...
    for file in fut_file_list:
        ...
        # fut_data_pre = fut_data.drop_vars(bnds_list).sel(time = fut_data.time.dt.year.isin(range(apply_yr_start, apply_yr_end + 1))) #Alicia
        ...
        fut_data_bc = adjust_attrs(fut_data_bc, var).chunk({'time': chunk_time, 'lat': chunk_lat, 'lon': chunk_lon}) #Alicia .transpose("time", "lat", "lon")

        for item in bnds_list:
            fut_data_bc[item] = fut_data[item]
            
        fut_data_bc.to_netcdf(path + f'{var}Adjust_AUST-20i_{gcm}_{"historical" if year_end <= 2014 else empat}_{mdl_run}_{org}_{rcm}_v1-r1-ACS-QME-{ref}-{train_yr_start}-{train_yr_end}_day_{year_start}0101-{year_end}1231.nc',encoding={var + 'Adjust': {'least_significant_digit': 2, 'zlib': True, 'complevel': 4}})#, compute = False),encoding={var + 'Adjust': {'chunksizes': (1, fut_data_bc.lat.size, fut_data_bc.lon.size), 'least_significant_digit': 2, 'zlib': True, 'complevel': 5}}
        # tasks.append(task)                          
    # dask.compute(*tasks)
    ...
    ######Alicia       
        ...
        # fut_data_bc.to_netcdf(path + f'{var}Adjust_AUST-20i_{gcm}_{"historical" if year_end <= 2014 else empat}_{mdl_run}_{org}_{rcm}_v1-r1-ACS-QME-{ref}-{train_yr_start}-{train_yr_end}_1hr_{year_start}0101-{year_end}1231.nc')#, compute = False) #Alicia
        ...
        # # path + f'{var}Adjust_AUST-20i_{gcm}_{"historical" if year_end <= 2014 else empat}_{mdl_run}_{org}_{rcm}_v1-r1-ACS-QME-{ref}-{train_yr_start}-{train_yr_end}_1hr_{year_start}0101-{year_end}1231_time0-23.nc',
        # # encoding=encoding,
        # # compute=False)
