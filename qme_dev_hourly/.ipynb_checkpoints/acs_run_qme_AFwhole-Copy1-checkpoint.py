...
if __name__ == "__main__":
    ...
    for file in fut_file_list:
        ...
        fut_data_bc.to_netcdf(path + f'{var}Adjust_{grid}_{gcm}_{"historical" if year_end <= 2014 else empat}_{mdl_run}_{org}_{rcm}_v1-r1-ACS-QME-{ref}-{train_yr_start}-{train_yr_end}_1hr_{year_start}0101-{year_end}1231.nc')#,encoding={var + 'Adjust': {'least_significant_digit': 2, 'zlib': True, 'complevel': 4}})#, compute = False),encoding={var + 'Adjust': {'chunksizes': (1, fut_data_bc.lat.size, fut_data_bc.lon.size), 'least_significant_digit': 2, 'zlib': True, 'complevel': 5}}
        # tasks.append(task)                          
    # dask.compute(*tasks)
