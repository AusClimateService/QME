...
outdir = "/g/data/xv83/users/at2708/bias_adjustment_acs_qme/qme_dev_copy/outputs/"
...
if __name__ == "__main__":
    ...
    if ref == "AGCDv1":
        ...
        ref_file_list = dataset_finder.get_datasets("ACS_DD", gcm = "ERA5", rcm = "BARRAR2", exact_match = True).select(var = var, year = dataset_finder.year_range(train_yr_start, train_yr_end), exact_match = True).get_files()
        
    else:
        raise ValueError("Unknown reference data")
    

    hist_dataset = dataset_finder.get_datasets("ACS_DD", org = org, gcm = gcm, scenario = ("historical", train_empat), mdl_run = mdl_run, rcm = rcm, timescale = 'day', exact_match = True).select(var = var, exact_match = True)
    ...
    fut_dataset = dataset_finder.get_datasets("ACS_DD", org = org, gcm = gcm, scenario = ("historical", empat), mdl_run = mdl_run, rcm = rcm, timescale = 'day', exact_match = True).select(var = var, exact_match = True)
    ...
    for file in fut_file_list:
        ...
        fut_data_pre = fut_data.drop_vars(bnds_list).chunk(chunks={"lat": chunk_size, "lon": chunk_size, "time": -1}).sel(time = fut_data.time.dt.year.isin(range(apply_yr_start, apply_yr_end + 1)))
        ...
        path = outdir + f'{org}/{gcm}/{"historical" if year_end <= 2014 else empat}/{mdl_run}/{rcm}/v1-r1-ACS-QME-{ref}-{train_yr_start}-{train_yr_end}/day/{var}Adjust/v20250311/'
        ...
        fut_data_bc.to_netcdf(path + f'{var}Adjust_AUST-05i_{gcm}_{"historical" if year_end <= 2014 else empat}_{mdl_run}_{org}_{rcm}_v1-r1-ACS-QME-{ref}-{train_yr_start}-{train_yr_end}_day_{year_start}0101-{year_end}1231.nc', 
                              encoding={var + 'Adjust': {'chunksizes': (1, fut_data_bc.lat.size, fut_data_bc.lon.size), 'least_significant_digit': 2, 'zlib': True, 'complevel': 5}})#, compute = False)
        # tasks.append(task)
