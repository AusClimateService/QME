...
# outdir = "/g/data/xv83/users/at2708/bias_adjustment_acs_qme/qme_dev_hourly/outputs_netcdf2netcdf_contiguous_wbgt20/"
# outdir = "/g/data/xv83/users/at2708/bias_adjustment_acs_qme/qme_dev_hourly/outputs_netcdf2netcdf_contiguous_wbgt11_conservative/"
outdir = "/g/data/xv83/users/at2708/bias_adjustment_acs_qme/qme_dev_hourly/test_hourly_correction/"
...
if __name__ == "__main__":
    ...
    hourly_bc = {}
    for hr in range(24):
        ref_hr = ref_data.where(ref_data.time.dt.hour == hr, drop=True)
        hist_hr = hist_data.where(hist_data.time.dt.hour == hr, drop=True)
    ...
    # Apply hourly bias correction to future data
    for file in fut_file_list:
        fut_data = xr.open_dataset(file)
        ...
        corrected_chunks = []
        for hr in range(24):
            fut_hr = fut_data_pre.where(fut_data_pre.time.dt.hour == hr, drop=True)
            
            if fut_hr.time.size > 0:
        ...
        fut_data_bc.to_netcdf(outfile, engine="h5netcdf")

    print("Hourly bias correction applied successfully.")
