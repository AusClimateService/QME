...
# outdir = "/g/data/xv83/users/at2708/bias_adjustment_acs_qme/qme_dev_copy_hourly/outputs_zarr2netcdf2/"
outdir = "/g/data/xv83/users/at2708/bias_adjustment_acs_qme/qme_dev_copy_hourly/outputs4/" #megamem96 instead of 48 above outdir
...
if __name__ == "__main__":
    ...
    if var in ("tasmax", "tasmin", "tas"):
        ...
        mdl_raw_means = xr.open_zarr(fut_file_list).chunk(chunks={"lat": chunk_lat, "lon": chunk_lon, "time": chunk_time}).drop_vars(bnds_list).groupby("time.year").mean("time")#.persist() #Zarr
    ...
    # ref_data_full = xr.open_mfdataset(ref_file_list, preprocess = preprocess_ds, parallel = True, engine='netcdf4',chunks={"time": chunk_time, "lat": chunk_lat, "lon":chunk_lon}) #Alicia
    ref_data_full = xr.open_zarr(ref_file_list).chunk(chunks={"lat": chunk_lat, "lon": chunk_lon, "time": chunk_time}).sel(time=slice("1980-01-01", "2022-12-31")) #Zarr
    ref_data_full=preprocess_ds(ref_data_full)
    hist_data_full = xr.open_zarr(hist_file_list).chunk(chunks={"lat": chunk_lat, "lon": chunk_lon, "time": chunk_time}).sel(time=slice("1980-01-01", "2022-12-31")) #Zarr
    ...
    ref_data = ref_data_full.drop_vars(bnds_list).chunk(chunks={"lat": chunk_lat, "lon": chunk_lon, "time": chunk_time}).sel(time = ref_data_full.time.dt.year.isin(range(train_yr_start, train_yr_end + 1))) #Alicia
    hist_data = hist_data_full.drop_vars(bnds_list).chunk(chunks={"lat": chunk_lat, "lon": chunk_lon, "time": chunk_time}).sel(time = hist_data_full.time.dt.year.isin(range(train_yr_start, train_yr_end + 1))) #Alicia
    ...
    fut_file_list_netcdf = fut_dataset.select(year = dataset_finder.year_range(apply_yr_start, apply_yr_end),remove_empty=False).get_files() #Alicia #Zarr
    for file in fut_file_list_netcdf:
            
        fut_data = xr.open_dataset(file)#, chunks = {'time': 1})#, chunks = {'time': 27, 'lat': -1, 'lon': -1})
        # fut_data_pre = fut_data.drop_vars(bnds_list).chunk(chunks={"lat": chunk_lat, "lon": chunk_lon, "time": chunk_time}).sel(time = fut_data.time.dt.year.isin(range(apply_yr_start, apply_yr_end + 1)))
        fut_data_pre = fut_data.drop_vars(bnds_list).chunk(chunks={"time": chunk_time, "lat": chunk_lat, "lon": chunk_lon}).sel(time = fut_data.time.dt.year.isin(range(apply_yr_start, apply_yr_end + 1))) #Alicia
        ...
        fut_data_bc = adjust_attrs(fut_data_bc, var).chunk({'time': chunk_time, 'lat': chunk_lat, 'lon': chunk_lon})#.transpose("time", "lat", "lon")#.chunk({'time': 12, 'lat': 5, 'lon': 5}) #Alicia

        for item in bnds_list:
            fut_data_bc[item] = fut_data[item]
            
        fut_data_bc.to_netcdf(path + f'{var}Adjust_AUST-05i_{gcm}_{"historical" if year_end <= 2014 else empat}_{mdl_run}_{org}_{rcm}_v1-r1-ACS-QME-{ref}-{train_yr_start}-{train_yr_end}_day_{year_start}0101-{year_end}1231.nc')#,encoding={var + 'Adjust': {'chunksizes': (1, fut_data_bc.lat.size, fut_data_bc.lon.size), 'least_significant_digit': 2, 'zlib': True, 'complevel': 5}})#, compute = False)
        # tasks.append(task)                          
    # dask.compute(*tasks)
    ...
    ######Alicia       
        ...
        # fut_data_bc.to_netcdf(path + f'{var}Adjust_AUST-05i_{gcm}_{"historical" if year_end <= 2014 else empat}_{mdl_run}_{org}_{rcm}_v1-r1-ACS-QME-{ref}-{train_yr_start}-{train_yr_end}_1hr_{year_start}0101-{year_end}1231.nc')#, compute = False) #Alicia
        ...
        # # path + f'{var}Adjust_AUST-05i_{gcm}_{"historical" if year_end <= 2014 else empat}_{mdl_run}_{org}_{rcm}_v1-r1-ACS-QME-{ref}-{train_yr_start}-{train_yr_end}_1hr_{year_start}0101-{year_end}1231_time0-23.nc',
        # # encoding=encoding,
        # # compute=False)
