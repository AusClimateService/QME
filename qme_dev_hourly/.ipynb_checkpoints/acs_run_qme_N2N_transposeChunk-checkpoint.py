...
outdir = "/g/data/xv83/users/at2708/bias_adjustment_acs_qme/qme_dev_hourly/outputs_netcdf2netcdf_transposeChunk/"
...
if __name__ == "__main__":
    ...
    if var in ("tasmax", "tasmin", "tas"):
        ...
        mdl_raw_means = xr.open_mfdataset(fut_file_list, preprocess=preproc_find_mean, parallel = True, engine='h5netcdf',chunks={"time": chunk_time, "lat": chunk_lat, "lon": chunk_lon}).persist() #Alicia 
    ...
    for file in fut_file_list:
        ...
        # fut_data_bc = adjust_attrs(fut_data_bc, var).chunk({'time': 27, 'lat': -1, 'lon': -1})
        fut_data_bc = adjust_attrs(fut_data_bc, var).transpose("time", "lat", "lon").chunk({'time': chunk_time, 'lat': chunk_lat, 'lon': chunk_lon}) #Alicia 
